...
export function ClinicDashboard({ recentReferrals }: ClinicDashboardProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Two giant CTAs side by side */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Refer to Medical Specialist */}
        <button
          type="button"
          onClick={() => setMedicalOpen(true)}
          className="group relative overflow-hidden rounded-3xl border border-teal-400/30 bg-gradient-to-br from-[#0f3b3a] via-[#0f5550] to-[#117a6f] p-6 lg:p-8 text-left shadow-xl shadow-teal-900/10 hover:shadow-2xl hover:shadow-teal-900/20 transition-all duration-300"
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_15%,rgba(45,212,191,0.22),transparent_55%)] pointer-events-none" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_85%,rgba(45,212,191,0.10),transparent_50%)] pointer-events-none" />

          <div className="relative flex flex-col gap-5">
            <div className="flex h-16 w-16 lg:h-20 lg:w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-400 to-emerald-500 shadow-lg shadow-teal-500/40 group-hover:scale-105 transition-transform duration-300">
              <Stethoscope className="h-8 w-8 lg:h-10 lg:w-10 text-white" strokeWidth={2.2} />
            </div>

            <div>
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="h-3.5 w-3.5 text-teal-200" />
                <span className="text-[11px] font-bold uppercase tracking-[0.18em] text-teal-200">Medical referral</span>
              </div>
              <h2 className="font-heading text-2xl lg:text-4xl font-bold text-white leading-tight tracking-tight">
                Refer to Medical Specialist
              </h2>
            ...
            <div className="flex items-center gap-2 self-start rounded-xl bg-white/10 group-hover:bg-white/20 px-4 py-2.5 ring-1 ring-white/20 transition-all duration-300">
        ...
        <button
          type="button"
          onClick={() => setLawyerOpen(true)}
          className="group relative overflow-hidden rounded-3xl border border-gold/30 bg-gradient-to-br from-[#1a2a4a] via-[#243456] to-[#2a3f6a] p-6 lg:p-8 text-left shadow-xl shadow-navy/10 hover:shadow-2xl hover:shadow-navy/20 transition-all duration-300"
        >
          ...
          <div className="relative flex flex-col gap-5">
            <div className="flex h-16 w-16 lg:h-20 lg:w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-[#d4a84b] to-[#c49a3f] shadow-lg shadow-gold/40 group-hover:scale-105 transition-transform duration-300">
              <Scale className="h-8 w-8 lg:h-10 lg:w-10 text-white" strokeWidth={2.2} />
            </div>

            <div>
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="h-3.5 w-3.5 text-gold" />
                <span className="text-[11px] font-bold uppercase tracking-[0.18em] text-gold">Legal referral</span>
              </div>
              <h2 className="font-heading text-2xl lg:text-4xl font-bold text-white leading-tight tracking-tight">
                Refer to Lawyer
              </h2>
              <p className="mt-2 text-sm text-white/70 leading-relaxed">
                Send a patient case to an attorney. Insurance and adjuster details are optional.
              </p>
            </div>
      ...
      <div className="rounded-2xl bg-white shadow-sm border border-gray-200/80 overflow-hidden">
        ...
        {recentReferrals.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">Use a button above to create your first referral.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: 'linear-gradient(to bottom, #fafbfc, #f5f6f8)' }}>
                  ...
                  <th className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-400">Kind</th>
                ...
                {recentReferrals.map((ref) => {
                  ...
                  return (
                    <tr key={ref.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-gray-700 text-xs">{destinationLabel(ref)}</td>
                      <td className="px-5 py-3.5">
                        <span
                          className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                            isMedical ? 'bg-teal-50 text-teal-700' : 'bg-amber-50 text-amber-700'
                          }`}
                        >
                          <span className={`h-1.5 w-1.5 rounded-full ${isMedical ? 'bg-teal-400' : 'bg-amber-400'}`} />
                          {isMedical ? 'Medical' : 'Legal'}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
