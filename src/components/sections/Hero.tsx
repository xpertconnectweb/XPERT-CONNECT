...
export function Hero({ data }: HeroProps) {
  const badge = data?.badge ?? 'Trusted by 10,000+ clients nationwide'
  const headlineLine1 = data?.headlineLine1 ?? 'Been in an'
  const headlineAccent = data?.headlineAccent ?? 'Accident?'
  const headlineLine3 = data?.headlineLine3 ?? 'We Can Help.'
  const subtitle = data?.subtitle ?? 'Connect with experienced attorneys and medical professionals who fight for your rights.'
  ...
  const ctaPrimaryText = data?.ctaPrimaryText ?? 'Call 1-844-XPERT-NOW'
  const ctaPrimaryHref = data?.ctaPrimaryHref ?? 'tel:+18449737866'
  const ctaSecondaryText = data?.ctaSecondaryText ?? 'Free Consultation'
      ...
      <div className="container relative z-10 mx-auto px-4 pt-32 pb-32">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap gap-6 lg:gap-10 justify-center">
            {trustBadges.map((tb) => (
              <div key={tb.text} className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/10 backdrop-blur-sm">
                  <tb.icon className="h-5 w-5 text-gold" />
                </div>
                <span className="text-sm font-medium text-white/80">{tb.text}</span>
              </div>
            ))}
          </div>
        </div>
